...
const SentimentChart: React.FC<SentimentChartProps> = ({ data, keyword, drilldownSentiment }) => {
  ...
  const legendPayload = (!drilldownSentiment ? SENTIMENT_ORDER : [drilldownSentiment])
      .filter(sentiment => SENTIMENT_ORDER.includes(sentiment.toUpperCase())) // Ensure valid sentiments
      .map(sentimentUpper => ({
          value: sentimentUpper.charAt(0).toUpperCase() + sentimentUpper.slice(1).toLowerCase(), 
  ...
  const getDataKey = (sentiment: string): keyof ChartDataPoint | null => {
      switch(sentiment.toUpperCase()) {
          ...
          case 'NEUTRAL': return 'avg_neutral';
          case 'MIXED': return 'avg_mix';
          default: return null;
      }
  }
  ...
  return (
    <div className="bg-gray-800 shadow-lg rounded-lg p-6 h-full flex flex-col"> 
      <h3 className="text-xl font-semibold mb-4 text-blue-300 shrink-0">
        Sentiment Trend: {keyword} 
        {drilldownSentiment && 
          <span className="text-base font-normal text-gray-400 ml-2">({drilldownSentiment} Only)</span>
        }
      </h3> 
              ...
              {linesToRender.map(sentiment => {
                  ...
                  return (
                      <Line 
                          ...
                          type="monotone" 
                          dataKey={dataKey} 
                          name={sentiment.charAt(0).toUpperCase() + sentiment.slice(1).toLowerCase()}
