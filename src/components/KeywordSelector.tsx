...
import { calculateControversialList } from "@/components/SentimentList";
...
const calculateKeywordsList = (data: DailySentimentData[]): string[] => {
  return [...new Set(data.map((item) => item.keyword))].sort();
};

export function KeywordSelector() {
  const [open, setOpen] = React.useState(false);
  const { dailyDataState, selectedKeyword, setSelectedKeyword } =
    useDailyData();
  const { data: dailyData, loading, error } = dailyDataState;

  useEffect(() => {
    if (!loading && dailyData.length > 0 && !selectedKeyword) {
      const controversialList = calculateControversialList(dailyData);
      const topControversialKeyword = controversialList[0]?.keyword;

      if (
        topControversialKeyword &&
        dailyData.some((item) => item.keyword === topControversialKeyword)
      ) {
        console.log(
          "Setting default keyword to top controversy:",
          topControversialKeyword
        );
        setSelectedKeyword(topControversialKeyword);
      }
    }
  }, [loading, dailyData, selectedKeyword, setSelectedKeyword]);
              ...
              {keywords.map((keyword) => (
                <CommandItem
                  ...
                  onSelect={(currentValue: string) => {
                    setSelectedKeyword(
                      currentValue === selectedKeyword ? null : currentValue
                    );
                    setOpen(false);
                  }}
